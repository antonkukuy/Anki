...
class LatexImporter(NoteImporter):
    ...
    needMapper = True
    needDelimiter = False
    # allowHTML always True (assignments have on affect):
    ...
    def processFile(self, fileString):
        ...
        # make all notes same length:
        self.numFields = max([len(note.fields) for note in self.noteList])
        for note in self.noteList:
            note.fields = note.fields + [""]*(self.numFields-len(note.fields))
    ...
    def processNote(self, noteString, globalTags):
        ...
        #self.processTags(post, newNote.tags)
