...
class MemriseImportWidget(QWidget):
	...
	def getLevelNotes(self, levelUrl):
		...
		def getAudioItems(parents):
			# include check for an "a" tag in the filter, since you can have an Audio field
			# with no audio link (says [Empty] on Memrise) and this would cause problems down the line
			filteredParents = filter(lambda p: p["class"] == "audio" and p.find("a"), parents)
			return map(lambda x: (x.find("a")["href"], self.AUDIO_NOTE), filteredParents)
		...
		colA.extend(getAudioItems(colAParents))
		...
		colB.extend(getAudioItems(colBParents))
	...
	def downloadWithRetry(self, url, tryCount):
		...
		try:
			...
			# so unfortunately all we can do is wait and retry.
			time.sleep(0.1)
